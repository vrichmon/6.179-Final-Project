...
bool onX(double x, double y){
	
	//hitting the floor or cieling
	if (y<2*R || y>(SCREEN_HEIGHT-2*R)){
		return true;
	}
	return false;
...
bool onY(double x, double y){
	
	//hitting the left or right wall
	if ( x<2*R || x>(SCREEN_WIDTH-2*R)){
		return true;
	}
	return false;
			...
			while( !quit ){
				...
				if (time == 1){ //if not the first time
					...
					for (y=0; y<numBalls; y++) { 
						...
						//if/else structures for handling bounces
						double temp, temp2;
						if(onY(array[y].currX, array[y].currY) && onX(array[y].currX, array[y].currY)  && array[y].escapeNum>6) { //hitting a corner
						...
						} else if(onY(array[y].currX, array[y].currY) && array[y].escapeNum>6){ //if hits y (vertical) walls, change the x sign
							
							temp = array[y].dx;
							array[y].dx= -1*temp;
							update(y);
							
						} else if (onX(array[y].currX, array[y].currY) && array[y].escapeNum>6){ //if hits x (horizontal floors/cieling), change the y sign
